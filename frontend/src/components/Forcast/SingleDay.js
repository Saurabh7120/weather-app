...
const SingleDay = ({data}) => {
    ...
    const today = `${format(parse('', '', new Date(data.date)), "EE, MMM d")}` === `${format(parse('', '', new Date()), "EE, MMM d")}`
    return (
        <HStack justifyContent={'space-evenly'} w={'100%'}>
            <Text w={'30%'}>{today ? "Today" : format(parse('', '', new Date(data.date)), "EE, MMM d")}</Text>
